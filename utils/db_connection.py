...
class DatabaseConnection:
    ...
    async def check_connection(self):
        ...
        if not self.__connection.is_connected():
            ...
            await cursor.execute(
                f"SELECT time, state from information_schema.tables where user='{os.environ['MYSQL_USER']}'")
    ...
    async def get_guild_channels(self, guild_id: int):
        cursor = await self.__connection.cursor()
        await cursor.execute(
            f"SELECT guild_channel_id FROM {os.environ['PROPS_TABLE_NAME']} WHERE guilds_id = {guild_id}")
    ...
    async def add_channel(self, guilds_id: int, channel_id: int, lang: str):
        cursor = await self.__connection.cursor()
        await cursor.execute(
            f"INSERT INTO {os.environ['PROPS_TABLE_NAME']} (guilds_id, guild_channel_id, guild_lang) VALUES ('{guilds_id}', '{channel_id}', '{lang}')")
        await self.__connection.commit()
    ...
    async def add_reactions(self, guild_id: int, channel_id: int, pass_reaction=None, wrong_reaction=None):
        cursor = await self.__connection.cursor()
        ...
        await self.__connection.commit()
        self.logger.info(f"Updated reactions for {guild_id}: {channel_id}")
    ...
    async def remove_channel(self, guilds_id: int, channel_id: int):
        cursor = await self.__connection.cursor()
        await cursor.execute(
            f"DELETE FROM {os.environ['PROPS_TABLE_NAME']} WHERE guilds_id={guilds_id} AND guild_channel_id={channel_id}")
        await self.__connection.commit()
        self.logger.info(f"{guilds_id} - {channel_id} props are deleted")
        print("Guild props are removed from database")

    async def remove_reactions(self, guilds_id: int, channel_id: int, pass_reaction: bool, wrong_reaction: bool):
        cursor = await self.__connection.cursor()
        if pass_reaction:
            await cursor.execute(
        ...
        await self.__connection.commit()
        self.logger.info(f"Removed reactions from {guilds_id} - {channel_id}")

    async def update_db(self, guilds_id: int, channel_id: int, last_sender_id: int, last_message: str, game_over: bool):
        cursor = await self.__connection.cursor()
        if not game_over:
            await cursor.execute(
                f"UPDATE {os.environ['PROPS_TABLE_NAME']} SET last_sender_id={last_sender_id}, last_word='{last_message}', streak_count = streak_count + 1 WHERE guilds_id={guilds_id} AND guild_channel_id={channel_id}")
            self.logger.debug(
                f"Updated props of {guilds_id}:{channel_id} to last sender:{last_sender_id}, last word:{last_message}, streak_count++")
            await cursor.execute(
            ...
            await cursor.execute(
                f"UPDATE {os.environ['PROPS_TABLE_NAME']} SET last_sender_id=NULL, last_word=NULL, streak_count=0 WHERE guilds_id={guilds_id} AND guild_channel_id={channel_id}")
    ...
    async def check_words(self, guild_id: int, channel_id: int, word: str):
        ...
        cursor = await self.__connection.cursor()
        await cursor.execute(
            f"SELECT COUNT(*) FROM {os.environ['WORDS_TABLE_NAME']} WHERE '{word}' IN (SELECT word FROM {os.environ['WORDS_TABLE_NAME']} WHERE guild_id = {guild_id} AND guild_channel_id = {channel_id})")
